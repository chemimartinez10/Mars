					...
					<div class="card">
						<div class="card-header">
							...
							<div class="card-tools">
								<button class="btn orangered" @click="chooseModal">
									Nuevo
			...
			<div class="modal-dialog modal-lg modal-dialog-centered" role="document">
				<div class="modal-content">
					...
					<form
						@submit.prevent="
              passes(
                editmode ? updateRecur() : createRecur()
              )
            "
					>
						<div class="modal-body">
							...
							<div class="form-row">
								...
								<div class="form-group col-md-6">
									<div class="input-group">
										...
										<select class="custom-select rounded-left" id="estado" v-model="form.estado">
							...
							<div class="form-row">
								<div class="form-group col-md-6">
									<div class="input-group">
										...
										<select class="custom-select rounded-left" id="color" v-model="form.color">
								...
								<div class="form-group col-md-6">
									<div class="input-group">
										...
										<textarea class="form-control" placeholder="Observación" v-model="form.observacion"></textarea>
									</div>
								</div>
							</div>
						</div>
						<div class="modal-footer">
							<button type="button" @click="backtoModal" class="btn btn-danger">
							...
							<div class="form-row">
								...
								<div class="form-group col-md-6">
									<div class="input-group">
										...
										<select class="custom-select rounded-left" id="color" v-model="form.color">
											<option value>Seleccione color</option>
											<option value="nuevo">Azul</option>
											<option value="bueno">Negro</option>
											<option value="dañado">Blanco</option>
										</select>
									</div>
								</div>
							</div>
...
export default {
	...
	methods: {
		...
		loadAll() {
			axios.get("api/articulo").then(({ data }) => (this.articulo = data.data));
			axios.get("api/equipo").then(({ data }) => {
				this.equipos = data.data;
				for (var i = 0; i < this.equipos.length; i++) {
					this.seriales[i] = this.equipos[i].serial;
				}
				Fuego.$emit("seriales", this.seriales);

			});
			axios.get("api/recurso").then(({ data }) => {
		...
		updateRecur() {
			this.$Progress.start();
			this.form
				.put("api/recurso/" + this.form.id_recurso)
				.then(() => {
					if (this.articulo === true) {
						this.form
							.put("api/articulo/" + this.form.id)
							.catch(() => {
								this.$Progress.fail();
							});
					} else {
						this.form.put("api/equipo/" + this.form.id).catch(() => {
							this.$Progress.fail();
						});
					}
					$("#addNewArt").modal("hide");
